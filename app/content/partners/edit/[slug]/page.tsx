...
export default function EditPartnerPage() {
    ...
    React.useEffect(() => {
        const fetchPartner = async () => {
            setIsLoading(true);
            try {
                const response = await partnerService.getPartner(slug as string);
                const partner = response.data;
                setPartnerId(partner.id);
                setName(partner.name);
                setAbout(partner.about || "");
                setWebsite(partner.website || "");
                setFocusAreas(partner.focus_areas || []);
                setImpact(partner.impact || []);
                if (partner.logo) {
                    setLogoPreview(partner.logo);
                }
            } catch (error: any) {
                toast.error(error.message || "Failed to fetch partner data");
                router.push("/content?tab=partners");
            } finally {
                setIsLoading(false);
            }
        };
        ...
        return (
            <DashboardLayout>
                <div className="flex items-center justify-center min-h-[400px]">
                    <p className="text-[#667085]">Loading partner data...</p>
                </div>
            </DashboardLayout>
        );
    }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-8 items-start">
                    ...
                    <div className="bg-white rounded-3xl border border-[#EAECF0] shadow-sm p-8 space-y-8">
                        <div>
                            ...
                            <p className="text-sm text-[#475467]">Update organization information and partnership details.</p>
                        </div>
