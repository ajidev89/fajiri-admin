...
export const campaignService = {
  ...
  urgentCampaigns(params?: Record<string, string>) {
    return apiClient.get<PaginatedResponse<Campaign>>("/campaigns/urgent", params);
  },
  ...
  createCampaign(payload: CreateCampaignPayload) {
    ...
    return apiClient.postFormData<ApiResponse<Campaign>>("/campaigns", formData);
  },
  ...
  updateCampaign(campaignId: string, payload: UpdateCampaignPayload) {
    ...
    if (payload.goal_amount) formData.append("goal_amount", payload.goal_amount);
    if (payload.status) formData.append("status", payload.status);
    if (payload.campaign_type) formData.append("campaign_type", payload.campaign_type);
    ...
    return apiClient.postFormData<ApiResponse<Campaign>>(`/campaigns/${campaignId}`, formData);
  },
};
